...
- `core/` — типы (`YdbPrimitive`), интерфейсы опций модуля (`YdbModuleOptions`, `YdbModuleAsyncOptions`, `YdbExecutor`), DI-токены (`constants.ts`), маппер TS→YDB, `quoteIdentifier`, `QueryOptions` (`trx`, `timeout`, `signal`, `limit`, `offset`). Retry-политика по типу ошибки (#27): `core/retry.ts` (классификация по структурным статусам YDB — только `ABORTED`/`UNAVAILABLE`/`OVERLOADED`; bounded exponential backoff + jitter; `runWithRetry`, `resolveYdbRetryPolicy`) и `core/retry-executor.ts` (`withRetryPolicy(executor, policy)` — гасит внутренний ретрай SDK через событие `retry` запроса: одна попытка SDK на попытку политики). Правило идемпотентности (fail-safe): политика ретраит только запросы, явно помеченные `.idempotent(true)`/`{ idempotent: true }` в QueryOptions (проброс — в executeQuery persistence/relations); непомеченные (в т.ч. все записи по умолчанию) выполняются ровно один раз. Опция `retry` есть в `YdbModuleOptions` (применяется в `createExecutor`) и в `runInTransaction().
