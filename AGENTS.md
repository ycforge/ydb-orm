...
- `entity/base-entity.ts` — `YdbBaseEntity` (Active Record): статические `find`, `findAll` (limit по умолчанию 100, макс 1000), `count`, `save` (upsert/update по PK, составной PK поддерживается во всех CRUD-операциях), `insertMany` (батчи по 100, внутри батча группировка по набору колонок — опущенные поля не затираются NULL), `loadRelations`. PK по умолчанию — `uuid`, автогенерация через `uuid` v4. Рантайм-зависимости (executor, провайдеры шифрования) — в `entity/entity-runtime.ts` (WeakMap по классу, наследники не разделяют состояние).
- `encryption/` — интерфейсы `YdbEncryptionProvider` / `YdbBlindIndexProvider` (encrypt/decrypt с AAD, hash для blind index). `@YdbEncrypted({ lazy: true })` — ленивая дешифровка: поле дешифруется не при SELECT, а явно через `decryptField()`/`decryptLazyFields()` на инстансе; `toJSON()` требует предварительной дешифровки. **Внимание:** `Base64TestEncryptionProvider` — заглушка для тестов, реальной криптографии нет.
...
- `module/` — интеграция с NestJS:
  ...
  - `ydb.module.ts` — `YdbModule.forFeature([...Entity])`: через `repository-factory.ts` сущностям инжектируется executor и провайдеры шифрования (без `forFeature` статические методы упадут с понятной ошибкой про `YdbModule.forFeature([...])` / `configureEntities([...])`). Кроме того, `forFeature` регистрирует инжектируемые `YdbRepository<Entity>` — токен получается через `getRepositoryToken(Entity)` или декоратор `@InjectRepository(Entity)`. `YdbRepository` — тонкая обёртка над статическими методами Active Record; также доступен `YdbEntityManager` (фабрика репозиториев).
