...
- **Идентичность только из контента (#169)**: файл миграции не может объявить собственный `hash` — загрузчик стирает его и всегда вычисляет заново из содержимого файла. Объявленный hash, отличающийся от контентного, — ошибка загрузки (признак подделки или устаревшего шаблона). Это поведение намеренно; см. «Обновление с версий до content-based identity» ниже.
...
### Обновление с версий до content-based identity

До появления content-based идентичности (#101) миграции сопоставлялись по имени, а загрузчик ещё в одной промежуточной версии позволял файлу объявить собственный `hash`, который перекрывал контентный (#169). При обновлении проекта с таких версий:

- **Проверьте, нет ли в файлах миграций объявленного `hash`** (например, `readonly hash = '...'` на классе или `hash: '...'` в объекте). Если есть — удалите это свойство.
- Если миграция с объявленным hash **уже применялась** при старой версии, её строка в `ydb_migrations` имеет id, производный от **объявленного** hash. После удаления свойства контентный hash не совпадёт с записью, и runner **откажется** выполнять такую миграцию повторно (ошибка «was modified after it was applied», в `migration:show` — `[#]`). Это защита от тихих повторных применений: silent corruption невозможен.
- Чтобы перейти на контентную идентичность безопасно: убедитесь, что изменения схемы от этой миграции реально в БД, затем вручную приведите запись к новому состоянию — `ydb-orm migration:repair <name> --as-applied` (или программно `runner.markMigrationApplied(<migration объект>)`). После этого запись будет сопоставлена уже с новым (контентным) хешем.
- Старые записи **без хеша** (самая первая версия, сопоставление по имени) поддерживаются и сейчас: они сопоставляются по имени, пока не будут перезаписаны reconcile'ом. Изменение содержимого такой миграции после применения — та же ошибка `modified`.
