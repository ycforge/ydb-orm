...
SDK ретраит одиночные запросы и тело транзакции **внутри себя** (неограниченный бюджет по умолчанию), поэтому ORM не дублирует это поведение автоматически — скрытого глобального состояния и умножения попыток нет. Вместо этого библиотека даёт явную политику `runWithRetry()` для составных операций вне транзакции, которые SDK как единое целое не ретраит:

```ts
import { runWithRetry } from '@ycforge/ydb-orm';

// Несколько шагов бизнес-логики: если любой из них упадёт транзитной
// ошибкой, будет повторена ВСЯ последовательность.
const result = await runWithRetry(async () => {
  ...
  maxAttempts: 5,     // максимум попыток ВКЛЮЧАЯ первую (по умолчанию 3)
  baseDelayMs: 100,   // база экспоненты (по умолчанию 100)
  maxDelayMs: 5_000,  // верхняя граница задержки (по умолчанию 5000)
  jitterRatio: 0.25,  // доля jitter 0..1 (по умолчанию 0.25)
  signal: controller.signal, // отмена ожидания и новых попыток
  onRetry: ({ attempt, error, delayMs }) => log.warn(`retry #${attempt} in ${delayMs}ms`, error),
});
```

**Какие ошибки повторяются.** Классификация структурная — по статус-кодам YDB (`@ydbjs/error`), текст сообщений не анализируется:

- **повторяются только** `ABORTED`, `UNAVAILABLE`, `OVERLOADED` (и `CommitError`, в причине которого такой статус);
- всё остальное пробрасывается немедленно без повтора: детерминированные ошибки приложения/валидации, `SCHEME_ERROR`/`BAD_REQUEST` и прочие статусы — включая те, что SDK считает условно-retryable (`SESSION_EXPIRED`, `UNDETERMINED`, `TIMEOUT`) и всегда-retryable (`BAD_SESSION`, `SESSION_BUSY`) — их повторами управляет сам SDK;
- при исчерпании попыток наружу выходит **последняя исходная ошибка** как есть (без заворачивания).

**Задержка** перед попыткой N: `min(baseDelayMs * 2^(N-1), maxDelayMs)`, затем jitter сжимает её в коридор `[(1 - jitterRatio) * raw, raw]`. Для тестов инъецируются `sleep(ms, signal)` и `rng()`.

**Где применять — важно.**

- ❌ Не оборачивайте одиночные запросы и `runInTransaction()`: их уже ретраит SDK внутри; вложение перемножит попытки (ваши × бесконечные SDK).
- ✅ Оборачивайте составные потоки вне транзакции (несколько запросов, шаги оркестрации). Колбэк обязан быть идемпотентным или устойчивым к повтору — те же требования, что к колбэку `idempotent`-транзакции (#98): побочные эффекты и lifecycle hooks выполнятся заново.
- Отмена (`signal.reason`) не превращается в повтор: операция завершается причиной отмены.

Для нестандартных обёрток ошибок есть точка расширения `shouldRetry(error)` (замещает классификацию по умолчанию). Утилиты `classifyYdbError()` / `isTransientYdbError()` доступны, если нужна только классификация.
