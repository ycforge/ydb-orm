...
- **Изоляция реестра сущностей**: сущности принадлежат приложению, которое их «приняло» при инициализации ядра; после `app.close()` набор сбрасывается, и следующее независимое приложение в том же процессе не наследует чужие сущности при schema sync (#142). Следствие: при повторном бутстрапе в том же процессе сущности нужно заново импортировать/зарегистрировать (`registerYdbEntity`) до старта — декоратор `@YdbEntity` регистрирует класс один раз за жизнь процесса. Вне Nest-приложения (CLI, standalone) sync/verify по-прежнему видят весь глобальный реестр.
