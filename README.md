...
`migration:check`, `migration:status` и `migration:show` используют единый **read-only** workflow: команды только читают состояние (`SELECT` из `ydb_migrations` + `DescribeTable` для сущностей) и ничего не меняют. Различимые состояния:
