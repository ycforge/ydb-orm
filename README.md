...
- **Изоляция реестра сущностей**: сущности принадлежат приложению, которое их «приняло» при инициализации ядра; после `app.close()` набор сбрасывается, и следующее независимое приложение в том же процессе не наследует чужие сущности при schema sync (#142). Повторный бутстрап в том же процессе работает автоматически: `YdbModule.forFeature([...])` при создании провайдеров явно привязывает объявленные сущности к скоупу своего приложения — декоратор `@YdbEntity` выполняется один раз за жизнь процесса (кеш модулей), поэтому восстановление видимости делает сама forFeature. Вне Nest-приложения (CLI, standalone) sync/verify по-прежнему видят весь глобальный реестр.
