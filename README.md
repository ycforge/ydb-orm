...
| `retry` на executor'е / `YdbModuleOptions.retry` | политика ORM: одна попытка SDK на попытку политики (внутренний цикл SDK гасится через событие `retry` запроса) | ровно `maxAttempts` обращений к БД |
...
**Требования идемпотентности** — те же, что у `idempotent`-транзакций (#98): при повторе заново выполняется вся операция (колбэк/запрос), побочные эффекты и lifecycle hooks могут сработать больше одного раза. Отмена (`signal.reason`) не превращается в повтор — операция завершается причиной отмены.
